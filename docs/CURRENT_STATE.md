...
This is not yet a market, competitor, relationship, account-management, or
executive intelligence product. Canonical client/donor **matching and cited
roll-up** exist (Phase 2); there is still no market digest, competitor table,
or calibrated win-probability model. Live org history is only as complete as
the unapplied organizations migration and stored proposal/win-loss rows.
Field-level ToR provenance exists; there is still no full provenance chain
from source page to **generated proposal claim**. A small golden set exists
for boolean-gate and scorer regression; live LLM extraction accuracy against
that set has not been run. Phase 3 (market intelligence) was not started.
...
introduced by Phase 2 and were not changed here. Golden extraction tests still
pass at the Phase 0/1 baseline (consultancy P/R/acc 1.000). There has been no
live `--once`, `--submit-url`, or `--run-assortis` execution as part of this
update. The `content_hash` and `organizations` migrations were not applied.
