...
- Anthropic API = all LLM calls (claude-opus-4-8 for everything)
...
1. OPENAI_API_KEY required for embeddings — text-embedding-3-small only
...
3. import os required at top of supabase_client.py — module-level
   anthropic.Anthropic(api_key=os.getenv(...)) needs it
...
This gate prevents burning Opus tokens on CV matching and proposal
...
Claude's is_consultancy_contract does the real quality filtering.
Do NOT make the RSS filter strict — it kills real opportunities.

## Models
CLAUDE_MODEL = "claude-opus-4-8"           # analysis + extraction
CLAUDE_MODEL_PROPOSAL = "claude-opus-4-8"  # proposal writing
